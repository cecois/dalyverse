    ...
    <div class="column is-12" style="padding-top:2em;">
      ...
      <div class="table-container">
        ...
        <table v-if="graph" class="table dv-index-table is-striped is-fullwidth">
          <tbody>
            <tr v-for="P in poetry.poems">
              <th>{{P.title}}</th>
              <td v-for="e in N.entities" :class="[e.daly=='true'?'has-text-weight-bold':'','dv-index-entities']" v-if="(!livefilter || e.label.toLowerCase().indexOf(livefilter)>=0)"><span v-if="e" @click.stop="setActive(e.id)" class="dv-trigger-active">{{P.title}}</span>
              </td>
